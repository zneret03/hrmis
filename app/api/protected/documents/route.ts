  ...
  try {
    ...
    const { data, error, count, totalPages, currentPage } =
      await paginatedData<AttendanceDB>({
        ...
        supabase,
        columns:
          'id, file, remarks, read_at, users!user_id!inner(email, username, id, employee_id), title, reason, certificate_type, data, certificate_status, created_at, updated_at, archived_at',
